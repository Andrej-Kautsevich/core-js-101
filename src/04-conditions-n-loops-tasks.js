...
function isCreditCardNumber(ccn) {
  const ccnS = ccn.toString().split('').reverse().join('');
  let sum = 0;
...
function isBracketsBalanced(str) {
  let string = str;
  while (string) {
    const reducedStr = string.replaceAll(/\[\]|\(\)|\{\}|<>/g, '');
    if (reducedStr === string) break;
    string = reducedStr;
  }
...
function getCommonDirectoryPath(paths) {
  const shortestPath = paths.reduce((prev, next) => {
  ...
  for (let i = 0; i < parts.length; i += 1) {
    const tmpPath = parts.slice(0, i + 1).join('/');
    if (paths.every((filePath) => filePath.startsWith(tmpPath))) {
      commonDir = tmpPath;
    } else {
      break;
    }
  }
  if (!commonDir && paths.every((filePath) => filePath.startsWith('/'))) {
    commonDir = '/';
    return commonDir;
  }
  return commonDir ? commonDir.concat('/') : '';
}
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m1.length; i += 1) {
    result.push(Array.from({ length: m2[0].length }, () => 0));
  }
